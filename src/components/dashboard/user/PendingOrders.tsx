    ...
    return (
        <div>
            {pendingData?.length ? (
                    ...
                    {pendingData.map((item: any, index: any) => (
                        ...
                        >
                            ...
                            <div className="px-2 sm:px-3 py-2 sm:py-3">
                                ...
                                <div className="space-y-1 sm:space-y-1.5 mb-2 sm:mb-3">
                                    ...
                                    <div className="flex items-center gap-1 sm:gap-1.5">
                                        ...
                                        <span className="text-[10px] sm:text-xs md:text-sm text-gray-600">
                                            {t('pendingOrders.date')}: {DateMaker(item?.createdAt)}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1 sm:gap-1.5">
                                        ...
                                        <span className="text-[10px] sm:text-xs md:text-sm text-gray-600">
                                            {t('pendingOrders.time')}: {item?.post?.time}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {/* Action Button */}
                                <div className="flex">
                                    <Link href={`/dashboard/user/my-order/track-progress/${item?.id}`} className="w-full">
                ...
                <div>
                    <h1>{t('pendingOrders.noAcceptedJob')}</h1>
                </div>
            )}
