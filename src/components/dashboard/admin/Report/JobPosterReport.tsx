...
import { useGetUserReportQuery } from '@/redux/api/admin/repost.api';
import Pagination from '@/components/shared/common/Pagination';
import { useTranslation } from 'react-i18next';
import Loader from '@/components/shared/Loader';

const JobPosterReport = () => {
    const { t } = useTranslation();
    const [currentPage, setCurrentPage] = useState(1);

    const { data, isLoading } = useGetUserReportQuery([
        { name: "limit", value: 10 },
    ...
    return (
        <div className="max-w-6xl space-y-4">
            {reportData?.length ? (
                            ...
                            <div className="flex items-start space-x-2 flex-1">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <h3 className="font-[500] text-[14px] text-[#17171A] mb-1">
                                        {t('jobPosterReport.report')} : {user?.reported?.fullName}
                                    </h3>
                                    <div className="mt-2">
                                        <p className="text-sm font-[400] text-[#17171A]">
                                            <span>{t('jobPosterReport.description')} : </span>{user.description}
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <div><h1>{t('jobPosterReport.noData')}</h1></div>
            )}
...
export default JobPosterReport;
