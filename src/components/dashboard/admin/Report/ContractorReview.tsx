...
import { useApproveReviewMutation, useGetAllContractorPendingReviewsQuery, useRejectReviewMutation } from '@/redux/api/publicePage/reviews.api';
...
const ContractorReview = () => {
    ...
    const { data, isLoading } = useGetAllContractorPendingReviewsQuery([
        { name: "limit", value: 10 },
    ...
    const contractorReviews = data?.data?.data;
    const metaData = data?.data?.meta;

    const handleApprove = async (id: any) => {
        const res = await approveReview(id);
        if (res?.data) toast.success(t('contractorReview.approveSuccess'));
        else toast.error(t('contractorReview.error'));
    };

    const handleReject = async (id: any) => {
        const res = await rejectReview(id);
        if (res?.data) toast.success(t('contractorReview.rejectSuccess'));
        else toast.error(t('contractorReview.error'));
    };
    ...
    return (
        <div className="max-w-6xl space-y-4">
            {contractorReviews?.length ? (
                contractorReviews.map((user: any) => (
                            ...
                            <div className="flex items-start space-x-2 flex-1">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <span className="text-sm font-[400] text-[#17171A]">
                                            {t('contractorReview.rating')}
                                        </span>
                                        <div className="flex space-x-1">{renderStars(user?.rating)}</div>
                                    </div>
                                    <div className="mt-2">
                                        <p className="text-sm font-[400] text-[#17171A]">
                                            <span>{t('contractorReview.review')} : </span>{user.comment}
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <div>
                    <h1>{t('contractorReview.noPending')}</h1>
                </div>
            )}
