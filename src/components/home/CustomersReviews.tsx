...
import { useGetAllCustomerReviewsQuery } from "@/redux/api/publicePage/reviews.api";
import testimonial_person from '@/assets/home/testimonial.jpg';
import { T } from "@/utils/translations";
import Loader from "../shared/Loader";

const CustomersReviews = () => {
    const { data, isLoading } = useGetAllCustomerReviewsQuery({});
    if (isLoading) {
            ...
            >
                {reviews.map((review: any) => (
                    <SwiperSlide key={review.id}>
                        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 bg-white p-6 rounded-xl">
                            ...
                            <div className="flex-1 flex flex-col justify-center gap-4">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center md:gap-6 mt-2">
                                    <div className="text-center md:text-left">
                                        <p className="text-lg md:text-xl font-semibold"><T>{review.reviewer.fullName}</T></p>
                                        <p className="text-sm md:text-base text-gray-600"><T>{review.reviewer.role}</T></p>
                                        <p className="text-sm md:text-base text-gray-600"><T>{review.reviewer.email}</T></p>
                                    </div>
                                </div>
                            </div>
                        </div>
