...
import { useMakePaypalForSubscriptionMutation } from "@/redux/api/payment/payment.api";
import { useGetSingleSubscriptionQuery } from "@/redux/api/publicePage/reviews.api";
import SubscriptionDetails from "@/components/home/SubscriptionDetails";
import { Card, CardContent } from "@/components/ui/card";
import { toast } from "sonner";

const BillingCheckout = () => {
    const [paymentMethod, setPaymentMethod] = useState("card");
    const [makePaypalPyament] = useMakePaypalForSubscriptionMutation()
    const { subsId } = useParams();
    console.log(subsId)
    const { data } = useGetSingleSubscriptionQuery(subsId)
        ...
        <div className="min-h-full p-6 pt-20">
            <div className="max-w-5xl mx-auto min-h-full">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    {/* Billing Details */}
                    <SubscriptionDetails courseData={orderData} />
                    {/* Payment Information */}
